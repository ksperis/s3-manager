...
class RGWAdminUserOperations:
    @staticmethod
    def _sanitize_uid(name: str) -> str:
        return re.sub(r"[^a-z0-9_.-]", "-", name.lower())

    @staticmethod
    def _to_rgw_bool(value: bool) -> str:
    ...
    def get_user(
        ...
        if result.get("not_found") and tenant:
            result = self._request(
                "GET",
                "/admin/user",
                params={"uid": f"{tenant}${uid}", "format": "json"},
                allow_not_found=allow_not_found,
            )
    ...
    def get_user_by_access_key(
        ...
        result = self._request(
            "GET",
            "/admin/user",
            params={"access-key": access_key, "format": "json"},
            allow_not_found=allow_not_found,
        )
        if result.get("not_found"):
            return None
    ...
    def extract_keys(self, data: Any) -> list[dict]:
        ...
        if isinstance(data, list):
            ...
            if isinstance(user_field, dict):
                entries.extend(self.extract_keys(user_field))
        ...
        prioritized = sorted(
            entries,
            key=lambda entry: 0 if entry.get("secret_key") else 1,
        )
        result: list[dict] = []
        ...
        for entry in prioritized:
            ...
            for field_name, field_value in entry.items():
                if field_name not in existing or existing.get(field_name) in (
                    None,
                    "",
                    [],
                    {},
    ...
    def list_users(self) -> list[Dict[str, Any]]:
        ...
        if not isinstance(result, list):
            return []
        return [
            entry if isinstance(entry, dict) else {"user": str(entry)}
            for entry in result
        ]
    ...
    def list_user_keys(
        ...
        return self.extract_keys(payload) if payload else []
