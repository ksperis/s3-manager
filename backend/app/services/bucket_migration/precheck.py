...
class BucketMigrationPrecheckPlanner:
    ...
    def _evaluate_item_safety(
        ...
        *,
        ...
        if requires_cutover:
            try:
                self._service._precheck_policy_roundtrip(source_context.account, item.source_bucket)
                add_check(
        ...
        if migration.lock_target_writes and requires_cutover:
            try:
                self._service._precheck_target_lock_with_probe_bucket(target_context, migration_id=migration.id)
                add_check(
        ...
        object_lock_configured = any(
            bool(object_lock.get(field)) for field in ("enabled", "mode", "days", "years")
        )
        delete_source_safe = True
        if migration.delete_source:
            if strategy == "version_aware" and object_lock_configured:
        ...
        rollback_safe = strategy in {"current_only", "skip_existing", "version_aware"} and not object_lock_configured
        if not rollback_safe:
            add_check(
                ...
                blocking=True,
                scope="rollback",
                message="Rollback safety cannot be guaranteed for this bucket with the current migration engine.",
            )
        return _ItemSafety(
