...
class TopicsService:
    ...
    def list_topics(self, account: S3Account) -> list[Topic]:
        ...
        raw_topics = sns_client.list_topics(access_key=access_key, secret_key=secret_key)
        items: list[Topic] = []
        for entry in raw_topics:
            ...
            if not arn:
                ...
                continue
            attrs = sns_client.get_topic_attributes(arn, access_key=access_key, secret_key=secret_key)
    ...
    def delete_topic(self, account: S3Account, topic_arn: str) -> None:
        access_key, secret_key = self._account_credentials(account)
        sns_client.delete_topic(topic_arn, access_key=access_key, secret_key=secret_key)

    def get_topic_policy(self, account: S3Account, topic_arn: str) -> Optional[dict]:
        access_key, secret_key = self._account_credentials(account)
        return sns_client.get_topic_policy(topic_arn, access_key=access_key, secret_key=secret_key)

    def set_topic_policy(self, account: S3Account, topic_arn: str, policy: dict) -> dict:
        access_key, secret_key = self._account_credentials(account)
        sns_client.set_topic_policy(topic_arn, policy, access_key=access_key, secret_key=secret_key)
        updated = self.get_topic_policy(account, topic_arn)
        return updated or {}

    def get_topic_configuration(self, account: S3Account, topic_arn: str) -> dict:
        access_key, secret_key = self._account_credentials(account)
        attrs = sns_client.get_topic_attributes(topic_arn, access_key=access_key, secret_key=secret_key)
