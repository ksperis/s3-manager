...
def _load_migration():
    migration_path = (
        ...
        / "alembic"
        / "versions"
        / "0094_canonical_s3_user_endpoints.py"
    )
    spec = util.spec_from_file_location("migration_0094_canonical_s3_user_endpoints", migration_path)
...
def test_migration_backfills_default_ceph_endpoint_and_enforces_not_null(monkeypatch):
    ...
    endpoints, users = _create_schema(engine)
    ...
    with engine.begin() as connection:
        ...
        assert connection.execute(
            sa.text("SELECT id, storage_endpoint_id FROM s3_users ORDER BY id")
        ).all() == [(10, 1), (11, 2)]
        columns = {column["name"]: column for column in sa.inspect(connection).get_columns("s3_users")}
        assert columns["storage_endpoint_id"]["nullable"] is False
        with pytest.raises(IntegrityError):
            with connection.begin_nested():
                connection.execute(users.insert().values(id=12, storage_endpoint_id=None))

        migration.downgrade()

        columns = {column["name"]: column for column in sa.inspect(connection).get_columns("s3_users")}
        assert columns["storage_endpoint_id"]["nullable"] is True
        connection.execute(sa.text("INSERT INTO s3_users (id, storage_endpoint_id) VALUES (13, NULL)"))


def test_migration_rejects_detached_users_without_a_default_ceph_endpoint(monkeypatch):
    engine = sa.create_engine("sqlite:///:memory:")
    endpoints, users = _create_schema(engine)

    with engine.begin() as connection:
        connection.execute(endpoints.insert().values(id=1, provider="other", is_default=True))
        connection.execute(users.insert().values(id=10, storage_endpoint_id=None))
        migration = _load_migration()
        _install_operations(monkeypatch, migration, connection)
