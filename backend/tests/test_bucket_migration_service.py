...
def test_run_item_dispatches_step_handlers_in_order(db_session):
    ...
    item = SimpleNamespace(
        execution_plan_json=_current_only_execution_plan(),
        source_bucket="bucket-a",
        target_bucket="bucket-b",
        status="running",
        step="copy_bucket_settings",
...
def test_run_item_rejects_unknown_step(db_session):
    ...
    try:
        ...
        assert False, "Expected an unsupported item step to fail"
    except RuntimeError as exc:
        assert str(exc) == "Unsupported item step: unknown"
