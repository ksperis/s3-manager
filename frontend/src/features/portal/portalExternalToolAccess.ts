...
export function buildGenericConnectionSheet(
  ...
  const lines = [
    ...
    `Bucket name: ${connection.bucketName}`,
    `Endpoint: ${connection.endpoint?.original || "Configured storage service"}`,
    `Access key: ${connection.key.access_key_id}`,
    `Permission: ${connection.permissionLabel}`,
    `Secret key: ${secret || "Not included in this file"}`,
    "",
    "Cyberduck",
    "- Import or double-click the .duck bookmark.",
    "- Enter the secret key when Cyberduck asks for the password.",
    "- The bookmark opens directly on the bucket above.",
    "",
    "Other S3 tools",
    "- Use the endpoint, bucket name, access key, and secret key above.",
