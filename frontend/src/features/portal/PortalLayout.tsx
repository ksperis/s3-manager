...
function AccountSelector() {
  ...
  const selectClasses =
    "appearance-none w-56 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:focus-visible:ring-offset-slate-900";
  const pillClasses =
    "w-56 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100";
  ...
  if (error) {
    return <div className="text-sm font-semibold text-rose-600">{error}</div>;
  }
  ...
  return (
    <div className="relative">
      ...
      <div className="pointer-events-none absolute inset-y-0 right-4 flex items-center text-xs text-slate-500 dark:text-slate-300">▼</div>
    </div>
  );
}
    ...
    <Layout
      ...
      topbarContent={
        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-3">
          <span className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Compte</span>
          <AccountSelector />
        </div>
      }
    >
