...
export default function ManagerCephKeysPage() {
  ...
  const tableStatus = resolveListTableStatus({ loading, error, rowCount: keys.length });

  if (!hasS3AccountContext) {
    return <div className="ui-body text-slate-600">Select an account before managing Ceph keys.</div>;
  }
    ...
    return (
      <div className="space-y-4">
        <PageHeader
          ...
          description="Manage Ceph RGW access keys for managed S3 users."
          breadcrumbs={[{ label: "Manager" }, { label: "Ceph" }, { label: "Access keys" }]}
        />
        <PageBanner tone="info">This page is available only for managed S3 User contexts.</PageBanner>
      </div>
    );
  }

  if (managerCephKeysEnabled === null) {
    return <div className="ui-body text-slate-600">Loading context...</div>;
  }
    ...
    return (
      <div className="space-y-4">
        ...
        <PageBanner tone="info">
          Ceph key management is unavailable for this context. Check feature toggle, endpoint provider, admin feature, and admin
          credentials.
        </PageBanner>
      </div>
    );
      ...
      <div className="ui-surface-card">
        <div className="border-b border-slate-200 px-4 py-3 dark:border-slate-800">
          ...
          <p className="ui-caption text-slate-500 dark:text-slate-400">
            The portal key is locked and cannot be disabled or deleted from this page.
          </p>
        </div>
