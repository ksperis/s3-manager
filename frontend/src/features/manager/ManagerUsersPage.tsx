  ...
  return (
    <div className="space-y-4">
      ...
      {createdKey && createdForUser && (
        <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-900/40 dark:bg-amber-950/50 dark:text-amber-100">
          <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link
              ...
              className="text-sm font-medium text-primary hover:text-sky-600 dark:text-primary-200 dark:hover:text-primary-100"
            >
              Manage keys
            </Link>
          </div>
          <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
            <div>
              <div className="text-[11px] uppercase tracking-wide text-amber-600 dark:text-amber-300">Access key</div>
            ...
            <div>
              <div className="text-[11px] uppercase tracking-wide text-amber-600 dark:text-amber-300">Secret key</div>
              <div className="rounded border border-amber-200 bg-white/80 px-3 py-2 font-mono text-xs text-slate-800 dark:border-amber-800 dark:bg-amber-50/10 dark:text-amber-100">
      ...
      <div className="rounded-2xl border border-slate-200/80 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="border-b border-slate-200 px-4 py-4 dark:border-slate-800">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-semibold text-slate-900 dark:text-slate-50">Users</p>
              <p className="text-xs text-slate-500 dark:text-slate-400">List plus key/policy actions.</p>
            </div>
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
              <span className="text-xs text-slate-500 dark:text-slate-400">{filteredUsers.length} user(s)</span>
        ...
        <Modal title="Create IAM user" onClose={closeAdvancedModal}>
          <form className="space-y-4" onSubmit={handleAdvancedCreate}>
            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold text-slate-700 dark:text-slate-200">User name</label>
              <input
                type="text"
                value={advancedName}
            ...
            <label className="flex items-center gap-2 rounded-md border border-slate-200 px-3 py-2 text-sm text-slate-600 dark:border-slate-700 dark:text-slate-300">
              <input
                type="checkbox"
                checked={createKey}
            ...
            <div className="space-y-2 rounded-xl border border-dashed border-slate-200/80 p-3 dark:border-slate-700">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <div className="text-sm font-semibold text-slate-800 dark:text-slate-100">Add to groups (optional)</div>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Launch permissions by linking groups before creation.</p>
                </div>
                <div className="flex items-center gap-2">
              ...
              {showGroupOptions && (
                <div className="flex flex-wrap gap-2">
                  {groups.length === 0 && <span className="text-sm text-slate-500 dark:text-slate-400">No groups available.</span>}
                  {groups.map((g) => {
            ...
            <div className="space-y-2 rounded-xl border border-dashed border-slate-200/80 p-3 dark:border-slate-700">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <div className="text-sm font-semibold text-slate-800 dark:text-slate-100">Attach policies (optional)</div>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Bind JSON policies now or skip and attach later.</p>
                </div>
                <div className="flex items-center gap-2">
              ...
              {showPolicyOptions && (
                <>
                  {policies.length === 0 ? (
                    <p className="text-xs text-slate-500 dark:text-slate-400">No policies available. Create them in the Policies tab.</p>
                  ) : (
                    <>
                      <input
                        type="text"
