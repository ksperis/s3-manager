...
const AWS_LEGACY_GLOBAL_S3_ENDPOINT = "https://s3.amazonaws.com";
const AWS_LEGACY_GLOBAL_STS_ENDPOINT = "https://sts.amazonaws.com";
...
function isAutoGeneratedAwsS3Endpoint(value: string): boolean {
  const normalized = normalizeEndpointValue(value);
  return (
    !normalized ||
    normalized === AWS_LEGACY_GLOBAL_S3_ENDPOINT ||
    /^https:\/\/s3\.[a-z0-9-]+\.amazonaws\.com(\.cn)?$/.test(normalized)
  );
}

function isAutoGeneratedAwsStsEndpoint(value: string): boolean {
  const normalized = normalizeEndpointValue(value);
  return (
    !normalized ||
    normalized === AWS_LEGACY_GLOBAL_STS_ENDPOINT ||
    /^https:\/\/sts\.[a-z0-9-]+\.amazonaws\.com(\.cn)?$/.test(normalized)
  );
}

function isAutoGeneratedAwsIamEndpoint(value: string): boolean {
  const normalized = normalizeEndpointValue(value);
  return (
    !normalized ||
    normalized === AWS_IAM_ENDPOINT ||
    normalized === AWS_GOV_IAM_ENDPOINT ||
    normalized === AWS_CN_IAM_ENDPOINT ||
    /^https:\/\/iam(\.[a-z0-9-]+)?\.amazonaws\.com(\.cn)?$/.test(normalized)
  );
}
  ...
  const handleRegionChange = (region: string) => {
    setForm((prev) => {
      ...
      const nextFeatures = applyFeatureConstraints(
        {
          ...
          sts: {
            ...prev.features.sts,
            endpoint: isAutoGeneratedAwsStsEndpoint(prev.features.sts.endpoint)
              ? awsStsEndpointForRegion(nextRegion)
              : prev.features.sts.endpoint,
          },
          iam: {
            ...prev.features.iam,
            endpoint: isAutoGeneratedAwsIamEndpoint(prev.features.iam.endpoint)
              ? awsIamEndpointForRegion(nextRegion)
              : prev.features.iam.endpoint,
          },
        },
        prev.provider
      );
      return {
        ...prev,
        region,
        endpoint_url: isAutoGeneratedAwsS3Endpoint(prev.endpoint_url)
          ? awsS3EndpointForRegion(nextRegion)
          : prev.endpoint_url,
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="space-y-1 ui-body font-semibold text-slate-700 dark:text-slate-100">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-slate-200 px-3 py-2 ui-body font-normal text-slate-900 shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-primary dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
                  placeholder={form.provider === "aws" ? awsS3EndpointForRegion(form.region) : "https://s3.example.com"}
                  required
                />
              </label>
              <label className="space-y-1 ui-body font-semibold text-slate-700 dark:text-slate-100">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <label className="space-y-1 ui-caption font-semibold text-slate-700 dark:text-slate-100">
                    ...
                    <input
                      ...
                      placeholder={form.provider === "aws" ? awsStsEndpointForRegion(form.region) : "https://sts.example.com"}
                  ...
                  <label className="space-y-1 ui-caption font-semibold text-slate-700 dark:text-slate-100">
                    ...
                    <input
                      ...
                      className="w-full rounded-lg border border-slate-200 px-3 py-2 ui-body font-normal text-slate-900 shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-primary disabled:opacity-60 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
                      placeholder={form.provider === "aws" ? awsIamEndpointForRegion(form.region) : "https://iam.example.com"}
