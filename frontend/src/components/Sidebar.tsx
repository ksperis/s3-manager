...
export default function Sidebar({ title = "s3-manager", sections, links = [], headerAction }: SidebarProps) {
  ...
  const baseLinkClasses =
    "group flex items-center justify-between rounded-md px-3 py-2 text-sm font-medium leading-5 transition-colors";
  ...
  const badgeClasses =
    "rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-semibold text-slate-600 group-hover:bg-primary-100 group-hover:text-primary-700 dark:bg-slate-800 dark:text-slate-300 dark:group-hover:bg-primary-800/50 dark:group-hover:text-primary-50";

  return (
    <aside className="hidden w-64 shrink-0 border-r border-slate-200/80 bg-white/95 px-3 pt-6 dark:border-slate-800 dark:bg-slate-900/70 md:flex md:flex-col">
      <div className="px-2 pb-4">
        <div className="flex flex-col gap-2">
          <div className="text-sm font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">{title}</div>
          {headerAction ? <div>{headerAction}</div> : null}
        </div>
      </div>
      <nav className="flex flex-1 flex-col gap-4 pb-8">
