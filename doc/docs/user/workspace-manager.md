...
2. Use **Storage** for buckets, the read-only lifecycle and bucket policy inventories, and manager browser (if enabled).
3. Use **IAM** for users, groups, roles, managed policies, and the read-only inline policy inventory (if IAM capability is available).
