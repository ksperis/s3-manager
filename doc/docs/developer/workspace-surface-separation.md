...
| --- | --- | --- | --- | --- |
| `/browser` standalone | Selected Browser context: S3 account, S3 connection, legacy S3 user, session context, or Portal account/Project context. | Classic root Browser uses `browser_enabled` and `browser_root_enabled`; Portal contexts use `browser_enabled`, `portal_enabled`, and `browser_portal_enabled` with `X-S3-Workspace: portal`. User or group `browser_advanced_features_enabled` controls advanced chrome on the root route. | Root Browser is enabled by default. When advanced Browser access is false, `/browser` uses Manager-equivalent compact chrome: no folder panel, no inspector panel, no action bar toggle, no root-only column/layout controls, and no bucket creation shortcut. Portal contexts use a user-oriented workspace sidebar, display Storage Space names instead of bucket names, and keep the `portal-basic` action profile. | Good default for technical users and Portal users who need a broader file workspace. Portal context must stay read/file-oriented and must not expose account-management features. |
...
Root `/browser` may also run in a Portal account or Project context. In that mode the
...
- Keep Portal authorization backed by Portal database metadata and grants.
  ...
  listings or roles.
- Project-scoped Portal access keys are grouped by Ceph zonegroup. They use one
  `project_iam_users` IAM identity per UI user, Project, and zonegroup, with
  rights projected from the Storage Space grants visible in that Project only.
  Do not duplicate keys per RGW Account in the same zonegroup and do not use
  Ceph Admin credentials to create, update, or revoke these keys.
...
| External S3 access after a user creates an account-scoped Portal access key. | The user's own Portal IAM key returned by `/portal/access-keys`. | The key is attached to the user's Portal IAM principal for the selected RGW Account. Portal-generated runtime keys remain hidden and protected from status changes or deletion. |
| External S3 access after a user creates a Project-scoped Portal access key. | The user's own Project IAM key returned by `/portal/projects/{projectId}/access-keys/{scopeId}`. | The key is attached to one `project_iam_users` principal for the UI user, Project, and Ceph zonegroup. Its inline policy is synchronized from DB-backed Storage Space grants across the RGW Accounts in that Project zonegroup. |
...
| Create, enable/disable, or delete a user-managed account-scoped Portal access key. | Stored RGW credentials of the selected `S3Account` through RGW IAM. | The backend mutates keys only on the current user's Portal IAM principal, enforces account settings and key limits, and rejects operations against the protected runtime key. |
| Create, enable/disable, or delete a user-managed Project-scoped Portal access key. | Stored RGW credentials of the Project zonegroup authority `S3Account` through RGW IAM. | The backend mutates keys only on the current user's `project_iam_users` principal for that Project and zonegroup, enforces Project Portal overrides and key limits, synchronizes the aggregated Storage Space policy, and records audit metadata with `project_id`, `scope_id`, `zonegroup`, and `authority_account_id`. It must not use Ceph Admin endpoint credentials. |
| Configure bucket-level replication from Portal. | Stored RGW credentials of the source and target `S3Account` records. | The user must have Portal manager rights, both Storage Spaces must be visible in the workspace, endpoints must be different locations in the same Ceph zonegroup, the source endpoint must declare the destination Ceph zone in `bucket_replication_target_zones`, and bucket replication must be allowed on both endpoints. RGW Account-owned buckets are hidden from configuration unless the endpoint declares `bucket_replication_owner_mode = rgw_account_supported`. The workflow enables versioning on source and target and writes the source bucket replication rule with audit metadata `executor = s3_account_admin`. It must not use Ceph Admin endpoint credentials. |
...
- External S3 credentials:
  `/portal/access-keys`, excluding the active Portal runtime key, or
  `/portal/projects/{projectId}/access-keys` for Project zonegroup scopes.
