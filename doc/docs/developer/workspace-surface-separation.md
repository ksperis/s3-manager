...
| --- | --- | --- | --- | --- |
| `/browser` standalone | Selected Browser context: S3 account, S3 connection, legacy S3 user, or session context. | `browser_enabled` and `browser_root_enabled`; user or group `browser_advanced_features_enabled` controls advanced chrome on the root route. | Root Browser is enabled by default. The folders/buckets panel is available on `/browser` and opens by default for fresh layouts. When advanced Browser access is false, `/browser` keeps compact chrome: no inspector panel, no action bar toggle, no root-only column/layout controls, and no bucket creation shortcut. | Good default for technical users. Consider disabling advanced access for day-to-day users who need bucket and folder navigation but not operator-focused Browser controls. |
...
| Advanced Browser chrome | Inspector details, layout persistence, action bar, column controls, and bucket shortcuts can make a simple file workflow look like an operator console. | Root route checks effective `browser_advanced_features_enabled` from the user or any UI group for advanced chrome. The root folders/buckets panel is treated as baseline `/browser` navigation. Embedded Browser sets `allowFoldersPanel`, `allowInspectorPanel`, and `showPanelToggles` to false. | Advanced chrome is disabled for embedded Manager, Portal, and Ceph Admin Browser. On `/browser`, users/groups without advanced Browser access keep the folders/buckets panel but lose inspector, action bar, root-only column/layout controls, and bucket creation. | Add more granular admin toggles only if compact/full is too coarse. |
