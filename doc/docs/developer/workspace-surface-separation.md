...
| Basic file mutations | Upload, folder creation, delete, and paste are expected for editors but not for read-only users. | Portal basic keeps upload, new folder, download, delete, and single-folder open visible; storage-side credentials decide whether the operation succeeds. Full profiles expose broader file mutations when context and state allow them. | Paste/copy/cut are disabled in Portal, but upload and delete are still visible in the minimal profile. | Add role-aware UI hiding for Portal Viewer or read-only connections while preserving backend/IAM enforcement. |
