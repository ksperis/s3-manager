...
| Technical Browser tools | Technical actions, advanced columns and search, versions, metadata mutation, batch operations, and bucket shortcuts can make a simple file workflow look like an operator console. | Root checks effective `browser_advanced_features_enabled`, presented to administrators as **Technical S3 tools**. This functional profile does not control display preferences. Every root `/browser` user can choose Compact or Comfortable and independently show Folders or the overlay Details panel. Root UI state v3 stores one set of root-only display preferences. Embedded surfaces receive their profile and density explicitly and never use root storage. | Technical tools are unavailable to Standard and Portal profiles. Root Browser defaults to Compact with both optional panels hidden; Manager and Ceph Admin embeds are compact, while the locked Portal embed is comfortable. Compact combines the path and icon actions on one row when width permits; Comfortable shows labeled actions on that row at wide viewports and moves them below the path before space becomes cramped. The Details panel exposes basic object and bucket facts to Standard users and adds versions, Ceph quotas, and feature states only in Advanced. Selection actions replace the stable context-bar actions in place instead of inserting another bar. | Add more granular admin toggles only if the technical bundle remains too broad. |
