...
| Advanced Browser chrome | Workbench panels, technical actions, advanced columns, and bucket shortcuts can make a simple file workflow look like an operator console. | Root checks effective `browser_advanced_features_enabled`, which grants the Advanced profile and optional Workbench. The functional profile controls actions, while density is a separate user choice for every root `/browser` profile. Root UI state v2 stores the shared density preference plus layout-specific Standard/Workbench preferences. Embedded surfaces receive profile, Standard layout, and density explicitly and never use root storage. | Workbench is unavailable to Standard and embedded Browser surfaces. Root Browser defaults to compact density when no preference exists; Manager and Ceph Admin embeds are compact, while the locked Portal embed is comfortable. Selection bars are automatic instead of a stored preference. | Add more granular admin toggles only if the Advanced profile remains too broad. |
