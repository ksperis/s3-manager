...
(`0.2.2`). They do not automatically follow an unpublished source checkout.
...
- stable image release: plain semver such as `0.2.2`
- stable minor series alias: `0.2` for the latest `0.2.x` release
- Git release tag: `v0.2.2`

## Which image tag should I use?

| Use case | Tag |
|---|---|
| Stable production-like deployment | plain semver such as `0.2.2` |
...
helm upgrade --install bucketreef helm/bucketreef \
  --set image.backend.repository=ghcr.io/ksperis/bucketreef-backend \
  --set image.backend.tag=0.2.2 \
  --set image.frontend.repository=ghcr.io/ksperis/bucketreef-frontend \
  --set image.frontend.tag=0.2.2
```
